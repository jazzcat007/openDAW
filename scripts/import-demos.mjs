...
const API_ROOT = "https://api.opendaw.studio/music"
...
const usage = `Usage:
  npm run import-demos [-- --root <path> --dry-run --replace]

Downloads the OpenDAW demo catalog and project bundles into a local factory mirror.

Options:
  --root <path>  Factory root. Defaults to FACTORY_MIRROR_ROOT or this install's factory path.
...
const main = async () => {
  ...
  const catalogResponse = await fetchOk(`${API_ROOT}/list-by-ids.php?ids=${ids.join(",")}`)
  ...
  let skipped = 0
  for (const id of ids) {
    const projectRoot = join(demosRoot, id)
    ...
    const bundle = Buffer.from(await (await fetchOk(`${API_ROOT}/uploads/${id}/project.odb`)).arrayBuffer())
    writeAtomically(projectPath, bundle)
    downloaded++
    console.log(`download ${id} (${bundle.length} bytes)`)
  }
